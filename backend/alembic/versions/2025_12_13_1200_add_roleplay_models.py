...
def upgrade():
    # Check if tables already exist (from Base.metadata.create_all)
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    existing_tables = inspector.get_table_names()

    # If any roleplay table exists, assume all do and skip migration
    roleplay_tables = [
        "roleplay_projects",
        "roleplay_characters",
        "roleplay_locations",
        "roleplay_wiki_entries",
        "roleplay_passages",
        "roleplay_threads",
        "dice_rolls",
    ]
    if any(table in existing_tables for table in roleplay_tables):
        return
    ...
    op.create_table(
        ...
        sa.Column(
            "has_character_sheets", sa.Boolean(), nullable=False, server_default="true"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["folder_ic_posts"], ["folders.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(["folder_ooc"], ["folders.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(
            ["folder_characters"], ["folders.id"], ondelete="SET NULL"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["folder_compiled"], ["folders.id"], ondelete="SET NULL"
        ),
    ...
    op.create_index(
        "ix_roleplay_projects_project_id", "roleplay_projects", ["project_id"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["roleplay_id"], ["roleplay_projects.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index("ix_roleplay_scenes_id", "roleplay_scenes", ["id"])
    op.create_index(
        "ix_roleplay_scenes_roleplay_id", "roleplay_scenes", ["roleplay_id"]
    )
    op.create_index(
        "idx_roleplay_scenes_sequence",
        "roleplay_scenes",
        ["roleplay_id", "sequence_number"],
    )
    ...
    op.create_index(
        "ix_roleplay_characters_roleplay_id", "roleplay_characters", ["roleplay_id"]
    )
    op.create_index(
        "ix_roleplay_characters_user_id", "roleplay_characters", ["user_id"]
    )
    op.create_index(
        "idx_roleplay_characters_active",
        "roleplay_characters",
        ["roleplay_id", "is_active"],
    )
    op.create_index(
        "idx_roleplay_characters_user",
        "roleplay_characters",
        ["roleplay_id", "user_id"],
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["scene_id"], ["roleplay_scenes.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["character_id"], ["roleplay_characters.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
            ["parent_passage_id"], ["roleplay_passages.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index("ix_roleplay_passages_id", "roleplay_passages", ["id"])
    op.create_index(
        "ix_roleplay_passages_roleplay_id", "roleplay_passages", ["roleplay_id"]
    )
    ...
    op.create_index(
        "idx_roleplay_passages_sequence",
        "roleplay_passages",
        ["roleplay_id", "sequence_number"],
    )
    op.create_index(
        "idx_roleplay_passages_scene",
        "roleplay_passages",
        ["scene_id", "sequence_number"],
    )
    op.create_index(
        "idx_roleplay_passages_user", "roleplay_passages", ["user_id", "created_at"]
    )
    ...
    op.create_index(
        "idx_lore_entries_category", "lore_entries", ["roleplay_id", "category"]
    )
    op.create_index(
        "idx_lore_entries_author", "lore_entries", ["author_id", "created_at"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["passage_id"], ["roleplay_passages.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_index(
        "ix_passage_reactions_passage_id", "passage_reactions", ["passage_id"]
    )
    op.create_index("ix_passage_reactions_user_id", "passage_reactions", ["user_id"])
    op.create_index(
        "idx_passage_reactions_unique",
        "passage_reactions",
        ["passage_id", "user_id"],
        unique=True,
    )
    op.create_index(
        "idx_passage_reactions_type",
        "passage_reactions",
        ["passage_id", "reaction_type"],
    )
    ...
    op.create_index(
        "idx_dice_rolls_roleplay", "dice_rolls", ["roleplay_id", "created_at"]
    )
    op.create_index(
        "idx_dice_rolls_character", "dice_rolls", ["character_id", "created_at"]
    )
